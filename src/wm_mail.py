...
def send_alert_for_user_count(hostname, data):
    subject = wm_config.config().get_option("send_email_subject_prefix") + wm_config.config().get_option("send_email_subject_suffix_uc") + " (" + hostname + ")"

    body = "Host: %s\n" % hostname
    for item in data:
        for key, value in item.items():
            body += "\nDatabase: %s, Current User Count: %d" % (key, value)
